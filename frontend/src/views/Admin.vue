<template>
  <div class="admin-container">
      ...
      <el-container>
        ...
        <el-main class="admin-main">
          <section v-if="activeMenu === 'users'" class="panel">
            ...
            <el-table :data="users" border stripe class="shadow-table" v-loading="loadingUsers">
              <el-table-column prop="id" :label="t('admin.users.id')" width="80" align="center" />
              <el-table-column prop="username" :label="t('admin.users.username')" min-width="160" />
              <el-table-column :label="t('admin.users.role')" width="120" align="center">
                <template #default="{ row }">
                  <el-tag :type="row.role === 'admin' ? 'danger' : 'primary'">
                    {{ row.role === 'admin' ? t('profile.roleAdmin') : t('profile.roleUser') }}
                  </el-tag>
                </template>
              </el-table-column>
              <el-table-column :label="t('admin.users.status')" width="120" align="center">
                <template #default="{ row }">
                  <el-tag :type="row.is_banned ? 'danger' : 'success'">
                    {{ row.is_banned ? t('common.status.banned') : t('common.status.active') }}
                  </el-tag>
                </template>
              </el-table-column>
              <el-table-column prop="created_at" :label="t('admin.users.createdAt')" min-width="200">
                <template #default="{ row }">
                  {{ formatDate(row.created_at) }}
                </template>
              </el-table-column>
              <el-table-column :label="t('common.actions.edit')" width="240" align="center">
                <template #default="{ row }">
                  <el-button
                    v-if="!row.is_banned"
                    type="warning"
          ...
          <section v-else-if="activeMenu === 'conversations'" class="panel">
            <header class="panel-header">
              ...
              <el-button :loading="loadingConversations" @click="loadConversations">
                <el-icon><Refresh /></el-icon>
                {{ t('common.actions.refresh') }}
              </el-button>
            </header>

            <el-table :data="conversations" border stripe class="shadow-table" v-loading="loadingConversations">
              <el-table-column prop="id" :label="t('admin.users.id')" width="80" align="center" />
              <el-table-column prop="title" :label="t('admin.conversations.title')" min-width="240" />
              <el-table-column prop="user_id" :label="t('admin.conversations.userId')" width="120" align="center" />
              <el-table-column :label="t('admin.conversations.status')" width="140" align="center">
                <template #default="{ row }">
                  <el-tag :type="row.is_active ? 'success' : 'info'">
                    {{ row.is_active ? t('common.status.active') : t('common.status.disabled') }}
                  </el-tag>
                </template>
              </el-table-column>
              <el-table-column prop="created_at" :label="t('admin.conversations.createdAt')" min-width="200">
                <template #default="{ row }">
                  {{ formatDate(row.created_at) }}
                </template>
              </el-table-column>
              <el-table-column :label="t('common.actions.view')" width="220" align="center">
                <template #default="{ row }">
                  <el-button size="small" @click="viewConversation(row)">
                    {{ t('admin.conversations.view') }}
                  </el-button>
                  <el-button type="danger" size="small" @click="deleteConversation(row.id)">
          ...
          <section v-else class="panel">
            ...
            <el-card class="settings-card" shadow="hover">
              <el-form ref="settingsFormRef" :model="settingsForm" :rules="settingsRules" label-width="140px" class="settings-form">
                <el-divider content-position="left">{{ t('admin.settings.basic') }}</el-divider>

                <el-form-item prop="website_name" :label="t('admin.settings.websiteName')">
                  <el-input v-model="settingsForm.website_name" :placeholder="t('admin.settings.websiteNamePlaceholder')" />
                </el-form-item>

                <el-form-item prop="website_logo" :label="t('admin.settings.websiteLogo')">
                  <div class="logo-upload-container">
                    ...
                    >
                      <el-button type="primary" :loading="uploadingLogo">
                        ...
                        {{ uploadingLogo ? t('admin.settings.uploading') : t('admin.settings.uploadLogo') }}
                      </el-button>
                    </el-upload>
                    <el-text size="small" type="info">{{ t('admin.settings.logoTips') }}</el-text>
                  </div>
                </el-form-item>

                <el-form-item prop="system_prompt" :label="t('admin.settings.systemPrompt')">
                  <el-input
                ...
                <el-divider content-position="left">{{ t('admin.settings.llm') }}</el-divider>
                ...
                <el-row :gutter="24" class="llm-grid">
                  <el-col :xs="24" :md="12">
                    <el-form-item prop="llm_provider" :label="t('admin.settings.provider')">
                      <el-select v-model="settingsForm.llm_provider" @change="handleProviderChange" class="full-width" placeholder="{{ t('admin.settings.providerPlaceholder') }}">
                  ...
                  <el-col :xs="24" :md="12">
                    <el-form-item prop="llm_base_url" :label="t('admin.settings.baseUrl')" :required="isBaseUrlRequired">
                      <el-input
                  ...
                  <el-col :xs="24" :md="12">
                    <el-form-item prop="llm_api_key" :label="t('admin.settings.apiKey')">
                      <el-input
                  ...
                  <el-col :xs="24" :md="12">
                    <el-form-item prop="llm_model_id" :label="t('admin.settings.modelId')">
                      <el-select
                  ...
                  <el-col :xs="24" :md="12">
                    <el-form-item prop="llm_model_name" :label="t('admin.settings.modelName')">
                      <el-input
    ...
    <el-dialog
      ...
      width="720px"
      :title="t('admin.conversations.dialogTitle', { title: selectedConversationTitle })"
    >
      <el-empty
        ...
        >
          <div class="message-bubble" :class="message.role">
            <h4>
              ...
              <span>{{ message.role === 'assistant' ? t('chat.assistantLabel') : t('chat.userLabel') }}</span>
            </h4>
      ...
      <template #footer>
        <el-button @click="conversationDialogVisible = false">{{ t('common.actions.close') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import { ElMessage, ElMessageBox, type FormInstance, type FormItemRule, type FormRules, type UploadFile } from 'element-plus'

import BackendStatus from '../components/BackendStatus.vue'
import MarkdownRenderer from '../components/MarkdownRenderer.vue'
import { adminAPI, type AdminSettingsResponse, type AdminSettingsUpdatePayload, type AdminUser, type ConversationMessage, type ConversationSummary, type LLMModelOption } from '../api/admin'
...
const settingsForm = reactive<AdminSettingsUpdatePayload & {
  llm_provider: string
  llm_model_id: string
...
const isBaseUrlRequired = computed(() => settingsForm.llm_provider === 'openaiful')

const baseUrlPlaceholder = computed(() => {
  if (settingsForm.llm_provider === 'openaiful') {
    return t('admin.settings.baseUrlRequiredPlaceholder')
  }
  const preset = providerDefaults[settingsForm.llm_provider as keyof typeof providerDefaults]
...
const settingsRules: FormRules = {
  website_name: [{ required: true, message: t('admin.settings.websiteNameRequired'), trigger: 'blur' }],
  system_prompt: [{ required: true, message: t('admin.settings.systemPromptRequired'), trigger: 'blur' }],
  llm_provider: [{ required: true, message: t('admin.settings.providerRequired'), trigger: 'change' }],
  llm_api_key: [{ required: true, message: t('admin.settings.apiKeyRequired'), trigger: 'blur' }],
  llm_model_id: [
    {
      validator: (_, value, callback) => {
        if ((value && value.trim().length > 0) || (settingsForm.llm_model_name && settingsForm.llm_model_name.trim().length > 0)) {
          callback()
        } else {
...
const setRuleMessage = (rules: FormRules[string], message: string) => {
  if (!rules) return
  const normalized: FormItemRule[] = Array.isArray(rules) ? [...rules] : [rules]
...
watch(isBaseUrlRequired, (required) => {
  if (required) {
    settingsRules.llm_base_url = [
      { required: true, message: t('admin.settings.baseUrlRequired'), trigger: 'blur' },
    ] as FormItemRule[]
  } else {
...
watch(locale, () => {
  setRuleMessage(settingsRules.website_name, t('admin.settings.websiteNameRequired'))
  setRuleMessage(settingsRules.system_prompt, t('admin.settings.systemPromptRequired'))
  setRuleMessage(settingsRules.llm_provider, t('admin.settings.providerRequired'))
  setRuleMessage(settingsRules.llm_api_key, t('admin.settings.apiKeyRequired'))
  setRuleMessage(settingsRules.llm_base_url, t('admin.settings.baseUrlRequired'))
})
...
const handleProviderChange = async (provider: string) => {
  ...
  if (provider !== previousProvider) {
    ...
    if (preset && (!settingsForm.llm_base_url || settingsForm.llm_base_url === providerDefaults[previousProvider as keyof typeof providerDefaults])) {
